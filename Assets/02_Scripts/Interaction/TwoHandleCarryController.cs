...
public sealed class TwoHandleCarryController : MonoBehaviour
{
    ...
    [Header("Grab Zones")]
    [SerializeField] private XRBaseInteractable _leftGrabZone;
    ...
    [Header("Carry Rule")]
    [SerializeField] private bool _requireDifferentPlayers = true;

#if UNITY_EDITOR
    [Header("Solo Test")]
    [SerializeField] private bool _soloTestTreatHandlesAsDifferentPlayers;
#endif
    ...
    private int ResolvePlayerId(IXRSelectInteractor interactor)
    {
        ...
        Debug.LogWarning($"[TwoHandleCarryController] Player id not found from interactor: {interactor.transform.name}", this);
        return NoPlayerId;
    }
